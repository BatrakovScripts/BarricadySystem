...
public OnPluginStart()
{
	...
	MAX_AMMO = CreateConVar("max_ammo_weapon", "999", "Количество патронов которое будет выдаваться при спавне объекта.", FCVAR_NOTIFY);
...
public Action:player_say(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (StrEqual(Text, "create"))
	{
		...
		AddMenuItem(menu, "option9", "Эмка");
		AddMenuItem(menu, "option10", "Автодробовик");
		AddMenuItem(menu, "option11", "Снайперка");
		AddMenuItem(menu, "option12", "Узишка");
		AddMenuItem(menu, "option13", "Помповый дробовик");
		SetMenuExitButton(menu, true);
		DisplayMenu(menu, client, MENU_TIME_FOREVER);
	}
		...
		{ 
			switch (itemNum)
			{
				...
				case 8:
				{
					...
					DispatchKeyValueVector(id, "Angles", Angles );
					DispatchSpawn(id);
					SetEntProp(id, Prop_Send, "m_iExtraPrimaryAmmo", GetConVarInt(MAX_AMMO), 4);
					PrintToChat(client, "Объект создан, порядковый ID:%d.", count[i]);
					count[i]++;
				}
				case 9:
